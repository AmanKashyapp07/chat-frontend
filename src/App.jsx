    ...
    <div className="flex items-center justify-center min-h-screen bg-[#EBE5DE] font-sans relative overflow-hidden">
      ...
      <div className="w-full max-w-md bg-white/60 backdrop-blur-2xl p-10 rounded-[2.5rem] shadow-[0_8px_30px_rgb(0,0,0,0.04)] border border-white/50 flex flex-col items-center z-10">
        ...
        <form onSubmit={handleSubmit} className="w-full space-y-4">
          ...
          {error && (
            <p className="text-rose-500 text-xs text-center font-medium mt-2 bg-rose-50 py-1 rounded-lg">
              {error}
            </p>
          )}
  ...
  return (
    <div className="h-screen flex flex-col bg-[#FDFBF7]">
      ...
      <div className="px-8 py-6 bg-white/80 backdrop-blur-md border-b border-[#EEE5DB] sticky top-0 z-10 flex justify-between items-center shadow-sm">
        <div>
          <h1 className="text-2xl font-serif font-bold tracking-tight text-[#4A3B32]">
            Chats
          </h1>
          <p className="text-xs font-medium text-[#9C8C7E]">
        ...
        <div className="flex gap-2">
          <button
            ...
            className="text-[#FFF8F0] bg-[#6F4E37] text-xs px-4 py-2 rounded-full hover:bg-[#5A3E2B] transition-all"
          >
            + New Group
          </button>
          <button
            onClick={onLogout}
            className="text-[#6F4E37] border border-[#6F4E37] text-xs px-4 py-2 rounded-full hover:bg-[#6F4E37] hover:text-white transition-all"
          >
            Logout
          </button>
        </div>
      ...
      <div className="flex-1 overflow-y-auto px-6 pt-4 space-y-2 pb-10">
        {loading ? (
          <div className="text-center mt-10 text-[#9C8C7E]">Loading...</div>
        ) : view === "users" ? (
          // USER LIST
          users.map((u) => (
            <div
        ...
        groups.length === 0 ? (
          <p className="text-center text-[#9C8C7E] mt-10">No groups yet.</p>
        ) : (
          groups.map((g) => (
            <div
              key={g.id}
            ...
            >
              ...
              <div className="flex-1">
                <h3 className="font-semibold text-[#4A3B32]">{g.name}</h3>
              </div>
            </div>
          ))
        )}
      ...
      {showCreateGroup && (
        <div className="fixed inset-0 bg-black/20 backdrop-blur-sm flex items-center justify-center z-50">
          <div className="bg-white p-6 rounded-[2rem] shadow-2xl w-full max-w-md animate-in zoom-in duration-200">
            <h2 className="text-xl font-bold text-[#4A3B32] mb-4">
              Create New Group
            </h2>
            ...
            <p className="text-sm font-bold text-[#9C8C7E] mb-2">
              Select Members:
            </p>
            ...
            <div className="flex gap-3">
              <button
                ...
                className="flex-1 py-3 text-[#6F4E37] font-bold hover:bg-[#F5F2EF] rounded-xl transition-colors"
              >
                Cancel
              </button>
              <button
                onClick={handleCreateGroup}
                className="flex-1 py-3 bg-[#6F4E37] text-white font-bold rounded-xl hover:bg-[#5A3E2B] transition-colors"
              >
                Create
              </button>
            </div>
  ...
  const handleFetchMembers = async () => {
    ...
    try {
      ...
      const response = await fetch(
        `http://localhost:8000/api/chats/group/fetch/${chatId}/members`,
        {
          headers: {
    ...
    <div className="flex flex-col h-screen animate-in slide-in-from-right duration-300 font-sans bg-[#FDFBF7]">
      ...
      <div className="px-6 py-4 bg-white/80 backdrop-blur-xl border-b border-[#EBE5DE] sticky top-0 z-20 flex items-center justify-between shadow-sm relative">
        <div className="flex items-center gap-4">
          <button
            onClick={onBack}
            className="w-10 h-10 flex items-center justify-center rounded-full bg-[#F5F2EF] text-[#6F4E37] hover:bg-[#EBE5DE] transition-colors"
          >
            <svg
              className="w-5 h-5"
              fill="none"
          ...
          <div className="flex flex-col">
            <span className="text-lg font-serif font-bold text-[#4A3B32]">
              {chatTitle}
            </span>
        ...
        {showMembers && type === "group" && (
          <div className="absolute top-16 left-16 z-50 w-64 bg-white shadow-xl rounded-xl border border-[#EBE5DE] p-4 animate-in fade-in zoom-in-95 duration-200">
            <div className="flex justify-between items-center mb-3 border-b border-[#F5F2EF] pb-2">
              <h3 className="font-bold text-[#4A3B32] text-sm">
                Group Members
              </h3>
              <button
                onClick={() => setShowMembers(false)}
                className="text-[#9C8C7E] hover:text-rose-500"
              >
                <svg
                  className="w-4 h-4"
                  fill="none"
            ...
            {isLoadingMembers ? (
              <div className="text-xs text-[#9C8C7E] p-2 text-center">
                Loading...
              </div>
            ) : (
        ...
        <div className="max-w-4xl mx-auto flex items-center gap-3 bg-[#F9F7F5] border border-[#EBE5DE] rounded-[1.5rem] px-2 py-2 focus-within:ring-2 focus-within:ring-[#6F4E37]/10 focus-within:border-[#D7C0AE] transition-all shadow-sm">
          ...
          <button
            ...
            className={`p-3 rounded-full transition-all duration-300 ${
              message.trim()
                ? "bg-[#6F4E37] text-[#FFF8F0] shadow-md hover:scale-105"
                : "bg-[#EBE5DE] text-[#FFF8F0] cursor-default"
            }`}
          >
